...
`;
  ...
  // Append the user's mimic prompt last so Whisper preserves it when its prompt window truncates.
  getWhisperPrompt(settings = getSettings()) {
    const basePrompt = mergeWhisperPrompt(
    ...
    const customPrompt = (settings.customTranscriptionPrompt || "")
      .trim()
      .replace(/\s+/g, " ");
    return [basePrompt, customPrompt].filter(Boolean).join(" ") || null;
  }
  ...
  async processWithLocalWhisper(
    ...
    try {
      ...
      // Add script bias, custom dictionary, and mimic prompt as the initial prompt.
      const transcriptionPrompt = this.getWhisperPrompt();
  ...
  async processWithOpenWhisprCloud(audioBlob, metadata = {}, wasCancelled = neverCancelled) {
    ...
    const transcriptionPrompt = this.getWhisperPrompt(settings);
  ...
  async processWithOpenAIAPI(audioBlob, metadata = {}, wasCancelled = neverCancelled) {
    ...
    try {
      ...
      if (proxySpec && !isSelfHostedTranscription(apiSettings)) {
        ...
        const transcriptionPrompt = this.getWhisperPrompt(apiSettings);
        ...
        const result = await call(
          proxySpec.buildPayload({
            ...
            language,
            apiSettings,
            dictionaryPrompt: transcriptionPrompt,
      ...
      let transcriptionPrompt = this.getWhisperPrompt(apiSettings);
      if (transcriptionPrompt) {
